...
UserRpcInfo::UserRpcInfo(const fastotv::user_id_t& uid, const fastotv::device_id_t& device_id)
    : user_id_(uid), device_id_(device_id) {}

bool UserRpcInfo::IsValid() const {
  return !user_id_.empty() && !device_id_.empty();
}

fastotv::device_id_t UserRpcInfo::GetDeviceID() const {
  return device_id_;
}

fastotv::user_id_t UserRpcInfo::GetUserID() const {
  return user_id_;
}
